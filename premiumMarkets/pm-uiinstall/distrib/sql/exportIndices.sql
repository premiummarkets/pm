...
select * from SHARES where symbol in ('FTSE','GDAXI','FCHI','IBEX','OSEAX','SSMI','NDX','BSESN','GSPC');

CALL SYSCS_UTIL.SYSCS_EXPORT_QUERY ('select * from shares where symbol in (''FTSE'',''FCHI'',''IBEX'',''OSEAX'',''SSMI'',''NDX'',''BSESN'')','/home/guil/Developpement/Quotes/tmp/SSSHARES.dat',null,null,null);
CALL SYSCS_UTIL.SYSCS_EXPORT_QUERY ('select * from quotations where symbol in (''FTSE'',''FCHI'',''IBEX'',''OSEAX'',''SSMI'',''NDX'',''BSESN'')','/home/guil/Developpement/Quotes/tmp/SSQUOTATIONS.dat',null,null,null);
