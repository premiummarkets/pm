...
select PERF_SUPPLEMENT.*, TUNEDCONF.*  from PERF_SUPPLEMENT join TUNEDCONF on  PERF_SUPPLEMENT.symbol = TUNEDCONF.symbol and PERF_SUPPLEMENT.isin=TUNEDCONF.isin where PERF_SUPPLEMENT.PERFDATE > TUNEDCONF.LASTCALCULATIONEND and  TUNEDCONF.CONFIGFILE='preTrainConfs.NEURAL.csv';
delete from PERF_SUPPLEMENT where PERF_SUPPLEMENT.PERFDATE > (select TUNEDCONF.LASTCALCULATIONEND FROM TUNEDCONF where PERF_SUPPLEMENT.symbol = TUNEDCONF.symbol and PERF_SUPPLEMENT.isin=TUNEDCONF.isin and TUNEDCONF.CONFIGFILE='preTrainConfs.NEURAL.csv');
...
select SYMBOL,ISIN,LATEST, count(*) from PERF_SUPPLEMENT where LATEST=1 group by  SYMBOL,ISIN,LATEST  having count(*) > 1;
select * from PERF_SUPPLEMENT where symbol in (select SYMBOL from PERF_SUPPLEMENT where LATEST=1 group by  SYMBOL,ISIN,LATEST  having count(*) > 1) and latest=1;
...
-- and also
select * from PERF_SUPPLEMENT where exists (select * from DUP_PORT_TMP where DUP_PORT_TMP.SYMBOL=PERF_SUPPLEMENT.SYMBOL and DUP_PORT_TMP.ISIN=PERF_SUPPLEMENT.ISIN) and PERF_SUPPLEMENT.NAME='UNKNOWN' and LATEST=1;
delete from PERF_SUPPLEMENT where exists (select * from DUP_PORT_TMP where DUP_PORT_TMP.SYMBOL=PERF_SUPPLEMENT.SYMBOL and DUP_PORT_TMP.ISIN=PERF_SUPPLEMENT.ISIN) and PERF_SUPPLEMENT.NAME='UNKNOWN' and LATEST=0;
select * from USERS where exists (select * from DUP_PORT_TMP where DUP_PORT_TMP.SYMBOL=USERS.SYMBOL and DUP_PORT_TMP.ISIN=USERS.ISIN) and USERS.NAME='UNKNOWN';
...
--SHARE duplicate SYMBOLS, ISIN
select * from SHARES where SYMBOL in (select SYMBOL from SHARES GROUP BY SYMBOL HAVING ( COUNT(SYMBOL) > 1 )) order by SYMBOL;
select * from SHARES where ISIN in (select ISIN from SHARES GROUP BY ISIN HAVING ( COUNT(ISIN) > 1 )) order by ISIN;
--Key SELECT SYMBOL, ISIN, COUNT(*) FROM SHARES GROUP BY SYMBOL, ISIN HAVING COUNT(*) > 1;
...
--Fix portfolio descrepencies V PERF Table
 ...
 --Users
-- Users with no prefs (no fix)
select USERS.SYMBOL, USERS.ISIN, USERS.NAME, USERS.EMAIL from USERS left join PERF_SUPPLEMENT on USERS.SYMBOL=PERF_SUPPLEMENT.SYMBOL and USERS.ISIN = PERF_SUPPLEMENT.ISIN and USERS.NAME=PERF_SUPPLEMENT.NAME where PERF_SUPPLEMENT.NAME is null; 
-- Users with no portfolio (to fix)
select USERS.SYMBOL, USERS.ISIN, USERS.NAME, USERS.EMAIL from USERS left join PORTFOLIO on USERS.SYMBOL=PORTFOLIO.SYMBOL and USERS.ISIN = PORTFOLIO.ISIN and USERS.NAME=PORTFOLIO.NAME where PORTFOLIO.NAME is null; 
-- Users with wrong portfolio (to fix)
select USERS.SYMBOL, USERS.ISIN, USERS.NAME, USERS.EMAIL from USERS left join PERF_SUPPLEMENT on USERS.SYMBOL=PERF_SUPPLEMENT.SYMBOL and USERS.ISIN = PERF_SUPPLEMENT.ISIN where USERS.NAME <> PERF_SUPPLEMENT.NAME and PERF_SUPPLEMENT.LATEST=1;
create table tmp2 select * from PERF_SUPPLEMENT where (symbol,isin) in (select USERS.SYMBOL , USERS.ISIN from USERS left join PERF_SUPPLEMENT on USERS.SYMBOL=PERF_SUPPLEMENT.SYMBOL and USERS.ISIN = PERF_SUPPLEMENT.ISIN where USERS.NAME <> PERF_SUPPLEMENT.NAME and PERF_SUPPLEMENT.LATEST=1) and PERF_SUPPLEMENT.LATEST=1;
update USERS join tmp2 on USERS.SYMBOL=tmp2.SYMBOL and USERS.ISIN = tmp2.ISIN set USERS.NAME=tmp2.NAME;
