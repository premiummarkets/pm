...
date PERF_SUPPLEMENT join (select A.* from PERF_SUPPLEMENT A join PERF_SUPPLEMENT as B on A.SYMBOL = B.SYMBOL and A.ISIN=B.ISIN  where A.NAME <> B.NAME and A.LATEST=1 group by A.SYMBOL, A.ISIN, A.NAME) as C on C.SYMBOL=PERF_SUPPLEMENT.SYMBOL and C.ISIN=PERF_SUPPLEMENT.ISIN set PERF_SUPPLEMENT.NAME=C.NAME;
