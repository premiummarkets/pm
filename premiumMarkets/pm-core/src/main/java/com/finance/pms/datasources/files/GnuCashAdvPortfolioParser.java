...
public class GnuCashAdvPortfolioParser {
	...
	private static final String[] expectedTitles = new String[]{"Account", "Symbol", "Basis", "Shares", "Price", "Money In", "Money Out", "Income", "Reali.ed Gain"};
	...
	private void checkTitles(Element row) throws XPathExpressionException {
		...
		for (int i = 0; i < thNodes.getLength(); i++) {
			String trimedColumnName = ((Element) thNodes.item(i)).getTextContent().trim();
			for (String expectedTitle : expectedTitles) {
				if (trimedColumnName != null && trimedColumnName.matches(expectedTitle)) {
		...
		for (String expcetedTitle : expectedTitles) {
			Integer titlePosition = titlesPositions.get(expcetedTitle);
			if (titlePosition == -1) {
				unfoundTitles.add(expcetedTitle);
			}
		}
