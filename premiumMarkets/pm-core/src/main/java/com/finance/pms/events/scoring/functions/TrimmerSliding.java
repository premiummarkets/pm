...
/**
 * Use @see com.finance.pms.events.operations.nativeops.calc.StatsOperation.class sliding normalisations instead 
 */
@Deprecated
public class TrimmerSliding {
...
//	private static MyLogger LOGGER = MyLogger.getLogger(Trimmer.class);
	...
	private Double slidingPeriod;

	// This is a lenient slider. FIXME incorporate in the Stats operation.
	public TrimmerSliding(Stock stock, Collection<QuotationDataType> quotationsDataTypes, Double slidingPeriod, Double trimFactor, Date start, Date end) {
		super();
		this.stock = stock;
	...
	public SortedMap<Date, double[]> trimmed(SortedMap<Date, double[]> data) throws NotEnoughDataException {
		...
		if (!Double.isNaN(slidingPeriod)) {	
			QuotationsFactories.getFactory().incrementDate(stock, quotationsDataTypes, startDateCal, +slidingPeriod.intValue());
			startDate = (startDateCal.getTime().compareTo(start) > 0) ? startDateCal.getTime() : start;
		}
		...
		for (Date date : subMap.keySet()) {
			...
			if (!Double.isNaN(slidingPeriod)) {
				...
				QuotationsFactories.getFactory().incrementDate(stock, quotationsDataTypes, currentDateCal, -slidingPeriod.intValue());
				periodStart = currentDateCal.getTime();
			}
//			currentDateCal.setTime(date);
...
//			Date periodEnd = currentDateCal.getTime();
			Collection<double[]> periodValues = MapUtils.subMapInclusive(subMap, periodStart, date).values();
