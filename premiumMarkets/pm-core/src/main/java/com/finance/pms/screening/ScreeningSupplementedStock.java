...
public class ScreeningSupplementedStock extends Validatable {
	...
	//Hibernate
	private ScreeningSupplementedStock() {
	}

	public ScreeningSupplementedStock(Stock stock) {
		this.stock = stock;
	...
	public void init(Date end) {
		...
		try {
			...
			close = firstEndDate.get(firstEndDate.size()-1).getCloseSplit();
			...
			ttmClose = firstTtmDate.get(firstEndDate.size()-1).getCloseSplit();
	...
	private MyBigDec ratingFormula(MyBigDec value, Double ideal) {
		...
		return new MyBigDec(value.getValue().divide(BigDecimal.valueOf(ideal),10, RoundingMode.HALF_EVEN).subtract(BigDecimal.ONE).abs(), true);
	}
	...
	private MyBigDec calculateEPSG(BigDecimal currentEPS, BigDecimal estEPS) {
		...
		return new MyBigDec(estEPS.subtract(currentEPS).divide(currentEPS, 10, RoundingMode.HALF_EVEN), true);
	}
	...
	public MyBigDec calculatePE(BigDecimal currentPrice, BigDecimal currentEPS) {
		...
		return new MyBigDec(currentPrice.divide(currentEPS, 10, RoundingMode.HALF_EVEN), true);
	}
	...
	private MyBigDec perCentOf(MyBigDec value, Integer factor) {
		...
		return new MyBigDec(value.getValue().divide(new BigDecimal(factor),10, RoundingMode.HALF_EVEN), true);
	}
	


	public void resetStock(ScreeningSupplementedStock completedStock) {
	...
	public int compareTo(Validatable o) {
		return this.stock.compareTo(((ScreeningSupplementedStock)o).getStock());
	}
	...
	private MyBigDec pricePotential(BigDecimal targetPrice) {
		...
		if (this.close != null && targetPrice != null && targetPrice.compareTo(BigDecimal.ZERO) != 0) {
			targetPercent = new MyBigDec(targetPrice.subtract(close).divide(close, 10, RoundingMode.HALF_EVEN), true);
		}
	...
	public BigDecimal yield() {
		...
		return this.getDividend().divide(this.close, 4, RoundingMode.HALF_EVEN);
	}
	...
	public MyBigDec payoutRatio() {
		...
		return new MyBigDec(div.divide(avgEps.getValue(),10, RoundingMode.HALF_EVEN), true);
	}
	...
	private MyBigDec avgEps() {
		...
		return new MyBigDec(epsSum.getValue().divide(new BigDecimal(nbv),10, RoundingMode.HALF_EVEN), true);
	}
	...
	public MyBigDec calculatePEG(MyBigDec pe, MyBigDec epsg) {
		...
		return new MyBigDec(pe.getValue().divide(epsg.getValue(), 2 , RoundingMode.HALF_EVEN).movePointLeft(2),true);
	}
	...
	public MyBigDec priceChangeTTM() {
		...
		if (this.priceChangeTTM  == null) {
			...
			if (this.close == null || ttmClose == null || ttmClose.compareTo(BigDecimal.ZERO) == 0) {
				...
				this.priceChangeTTM = new MyBigDec(priceDiffTTM.divide(ttmClose, 10, RoundingMode.HALF_EVEN), true);
			}
		}
	...
	public MarketValuation getMarketValuation() {
		return this.stock.getMarketValuation();
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ScreeningSupplementedStock other = (ScreeningSupplementedStock) obj;
		if (stock == null) {
			if (other.stock != null)
				return false;
