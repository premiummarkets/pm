...
public class HttpSourceHousePrices extends HttpSource implements SourceConnector {
	
	public HttpSourceHousePrices(String pathToprops, Providers beanFactory) {
	...
	public MyUrl getStockQuotationURL(String ticker, String startYear, String startMonth, String startDay, String endYear, String endMonth, String endDay) {
		...
		String region = ticker;
		//London: https://landregistry.data.gov.uk/app/ukhpi/download/new.csv?from=1950-01-01&to=2024-03-01&location=http%3A%2F%2Flandregistry.data.gov.uk%2Fid%2Fregion%2Flondon
		//Uk: "from="+start+"&to="+end+"&location=http%3A%2F%2Flandregistry.data.gov.uk%2Fid%2Fregion%2Funited-kingdom"
		return new MyUrl("https://landregistry.data.gov.uk/app/ukhpi/download/new.csv?"
				+ "from="+start+"&to="+end+"&location=http%3A%2F%2Flandregistry.data.gov.uk%2Fid%2Fregion%2F" + region);
	}
