...
public class EqualDoubleMapCondition extends CmpDoubleMapCondition {
	...
	private EqualDoubleMapCondition() {
		super("data equality", "True when the first time series value is equal the second one.");
	}
	...
	public Boolean conditionCheck(@SuppressWarnings("unchecked") Comparable<Double>... ops) {
		...
		return
				secondV < firstV && (firstV-secondV)/secondV <= epsilonMaxError ||
				firstV < secondV && (secondV-firstV)/firstV <= epsilonMaxError;
	}
